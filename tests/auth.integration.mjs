...
try {
  ...
  const proposal = { action: 'createProposal', taskId: created.id, teamName: 'Same team name', solutionIdea: 'A concrete solution idea for the business', plan: 'A concrete project plan with several delivery stages', estimatedDuration: '2 weeks', prototypeUrl: 'https://example.com/prototype', studentId: otherStudent.user.id };
  ...
  status(await request('/api/state', { cookie: student.cookie, body: { ...proposal, prototypeUrl: '' } }), 400, 'prototype URL is required');
  ...
  assert.equal(sentState.proposals[0].prototypeUrl, proposal.prototypeUrl);
  const second = await request('/api/state', { cookie: student.cookie, body: { ...proposal, solutionIdea: 'A second independent solution proposal for the same task' } });
  ...
  const third = await request('/api/state', { cookie: student.cookie, body: { ...proposal, solutionIdea: 'A third solution that the business can reject manually' } });
