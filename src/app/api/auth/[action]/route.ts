...
export async function POST(request: Request, context: { params: Promise<{ action: string }> }) {
  ...
  if (action === "register") {
    ...
    const result = sqlite.prepare(`INSERT INTO accounts (login, name, role, password_hash, created_at)
      VALUES (?, ?, ?, ?, ?) ON CONFLICT(login) DO NOTHING`).run(login, data.name, role, passwordHash, Date.now());
    if (!result.changes) return fail("login_taken", 409);
    user = { id: Number(result.lastInsertRowid), login, name: data.name, role };
  } else {
    ...
    if (!account || !valid || account.role !== role) return fail("invalid_credentials", 401);
