...
import { ArrowRight, Bot, BriefcaseBusiness, Check, LogOut, CircleAlert, Filter, Languages, Lightbulb, Loader2, Send, Sparkles, Users } from "lucide-react";
...
type View = "catalog" | "create" | "proposals";
...
export default function Dashboard({ user }: { user: SessionUser }) {
  ...
  const navigation = <>
    <Nav active={view === "catalog"} click={() => setView("catalog")}>{t.catalog}</Nav>
    {role === "business" && <Nav active={view === "create"} click={() => setView("create")}>{t.create}</Nav>}
    <Nav active={view === "proposals"} click={() => setView("proposals")}>{t.proposals}</Nav>
  </>;
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95">
      <div className="mx-auto flex min-h-16 max-w-7xl flex-wrap items-center gap-3 px-5 py-3 lg:gap-8">
        <button onClick={() => setView("catalog")} className="flex items-center gap-2 font-black"><span className="grid size-9 place-items-center rounded-xl bg-indigo-600 text-white"><Sparkles size={18} /></span>SteppeMind</button>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <span className="control max-w-48" title={user.name}>{role === "business" ? <BriefcaseBusiness size={16} /> : <Users size={16} />}<span className="truncate">{user.name}</span><span className="sr-only">{role === "business" ? t.business : "Студент"}</span></span>
          <button onClick={logout} disabled={leaving} className="control"><LogOut size={16} />{locale === "ru" ? "Выйти" : "Шығу"}</button>
        </div>
      </div>
    ...
    <main className="mx-auto max-w-7xl px-5 py-10">
      ...
      {loading ? <div className="py-20 text-center"><Loader2 className="mx-auto animate-spin" />{t.loading}</div> : <>
        ...
        {view === "create" && role === "business" && <Builder t={t} locale={locale} done={async () => { await refresh(); setView("catalog"); }} />}
...
function Nav({active,click,children}:{active:boolean;click:()=>void;children:React.ReactNode}){return <button onClick={click} className={`rounded-lg px-3 py-2 text-sm font-semibold ${active?"bg-indigo-50 text-indigo-700":"text-slate-600"}`}>{children}</button>}
...
function ProposalList({t,state,role,refresh}:{t:typeof copy.ru|typeof copy.kk;state:State;role:Role;refresh:()=>Promise<void>}){const [error,setError]=useState(false),[busy,setBusy]=useState(false);async function decide(id:number,status:"accepted"|"rejected"){setBusy(true);setError(false);try{const response=await apiFetch("/api/state",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"proposalStatus",id,status})});if(!response.ok)throw new Error("Decision failed");await refresh()}catch{setError(true)}finally{setBusy(false)}}const list=state.proposals;return <div><span className="eyebrow">{t.proposals}</span><h1 className="mt-4 text-4xl font-black">{role==="business"?t.incoming:t.proposals}</h1>{role==="business"&&<p className="mt-3 text-slate-600">{t.incomingText}</p>}{error&&<p role="alert" className="mt-4 text-red-600">{t.error}</p>}<div className="mt-8 space-y-4">{!list.length&&<div className="card p-10 text-center">{t.empty}</div>}{list.map(p=><article key={p.id} className="card p-6"><div className="flex justify-between"><div><span className="tag">{p.teamName}</span><h2 className="mt-3 text-xl font-black">{state.tasks.find(x=>x.id===p.taskId)?.title}</h2></div><span className="text-sm font-bold">{t[p.status as "accepted"|"rejected"|"pending"]}</span></div><p className="mt-4 text-sm text-slate-600">{p.solutionIdea}</p><p className="mt-2 text-sm"><b>{t.plan}:</b> {p.plan}</p>{role==="business"&&p.status==="pending"&&<div className="mt-5 flex gap-2"><button disabled={busy} onClick={()=>decide(p.id,"accepted")} className="primary"><Check/>{t.accept}</button><button disabled={busy} onClick={()=>decide(p.id,"rejected")} className="secondary">{t.reject}</button></div>}</article>)}</div></div>}
