...
export default function Dashboard({ user, initialView = "catalog", initialTaskId }: { user: SessionUser; initialView?: View; initialTaskId?: number }) {
  ...
  return <div className="min-h-screen bg-[#f6f7f2] text-[#17221d]">
    <header className="sticky top-0 z-20 border-b border-[#dfe6df] bg-[#fdfefb]/95 backdrop-blur">
      <div className="mx-auto flex min-h-16 max-w-7xl flex-wrap items-center gap-3 px-5 py-3 lg:gap-8">
        <Link href="/catalog" className="flex items-center gap-2 font-black tracking-tight"><span className="grid size-9 place-items-center rounded-xl bg-[#157a62] text-white"><Sparkles size={18} /></span>SteppeMind</Link>
...
function Catalog({locale,industries,t,tasks,teamProfiles,proposalCounts,filter,setFilter,role,userId,userName}:{locale:Locale;industries:string[];t:typeof copy.ru|typeof copy.kk;tasks:Task[];teamProfiles:TeamProfile[];proposalCounts:Record<number,number>;filter:string;setFilter:(x:string)=>void;role:Role;userId:number;userName:string}){const router=useRouter();const [selected,setSelected]=useState<Task|null>(null);
  ...
  return <><section className="mb-10 overflow-hidden rounded-2xl bg-[#123b32] px-8 py-10 text-white shadow-[0_18px_40px_rgba(18,59,50,.16)] md:px-12 md:py-12"><span className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#9bd1b9]/25 bg-[#9bd1b9]/10 px-3 py-1 text-xs font-bold text-[#bce5cf]"><Sparkles size={13}/>{t.tagline}</span><h1 className="max-w-3xl text-4xl font-black tracking-tight md:text-5xl">{t.hero}</h1><p className="mt-4 max-w-2xl leading-7 text-[#c5d8cf]">{t.heroText}</p></section><div className="mb-6 flex flex-wrap items-center gap-2"><Filter size={17} className="mr-1 text-[#64736c]"/>{["all","draft","workable","ready","priority"].map(k=><button key={k} onClick={()=>setFilter(k)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${filter===k?"bg-[#157a62] text-white shadow-sm":"border border-[#dfe6df] bg-white text-[#53635b] hover:border-[#9dc5b4]"}`}>{t[k as "all"|"draft"|"workable"|"ready"|"priority"]}</button>)}</div><div className="mb-6 flex flex-wrap gap-4">
