...
import { ArrowRight, Bot, CalendarDays, Check, ChevronDown, CircleAlert, ExternalLink, Filter, Languages, LayoutDashboard, Lightbulb, Loader2, LogOut, Sparkles, UserRound } from "lucide-react";
...
export default function Dashboard({ user, initialView = "catalog" }: { user: SessionUser; initialView?: View }) {
  ...
  const navigation = <>
    ...
    <Nav href={proposalsHref} active={view === "proposals"}>{role === "team" ? t.myProposals : t.proposals}</Nav>
  </>;
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
    ...
    <main className="mx-auto max-w-7xl px-5 py-10">
      ...
      {loading ? <div className="py-20 text-center"><Loader2 className="mx-auto animate-spin" />{t.loading}</div> : <>
        ...
        {view === "catalog" && <Catalog t={t} locale={locale} tasks={tasks} proposalCounts={state.proposalCounts} filter={filter} setFilter={setFilter} role={role} userName={user.name} onProposalSubmitted={async () => { await refresh(); router.push("/student/proposals"); }} />}
...
function DashboardHome({t,locale,user,state}:{t:typeof copy.ru|typeof copy.kk;locale:Locale;user:SessionUser;state:State}) {
  ...
  return <div>
    ...
    <section className="mt-8">
      ...
      {isBusiness ? <TaskCards tasks={ownTasks} proposalCounts={state.proposalCounts} t={t}/> : <ProposalList t={t} locale={locale} state={state} role="team" refresh={async()=>{}} compact/>}
    </section>
  </div>;
}
...
function TaskModal({task,userName,role,locale,t,close,done}:{task:Task;userName:string;role:Role;locale:Locale;t:typeof copy.ru|typeof copy.kk;close:()=>void;done:()=>Promise<void>}){
  return <div role="dialog" aria-modal="true" aria-labelledby="task-title" className="fixed inset-0 z-40 grid place-items-center bg-slate-950/50 p-4" onMouseDown={close}>
    <div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-3xl bg-white p-7" onMouseDown={event=>event.stopPropagation()}>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {(["need","users","dataMaterials","constraints","expectedResult","successCriteria","contact","interactionFormat"] as const).map(key=><div key={key} className="rounded-xl bg-slate-50 p-4">
          <p className="mb-1 text-xs font-bold uppercase text-slate-400">{t.fields[key]}</p>
          <p className="text-sm leading-6">{task[key]||"—"}</p>
        </div>)}
      </div>
...
function ProposalList({t,locale,state,role,refresh,compact=false}:{t:typeof copy.ru|typeof copy.kk;locale:Locale;state:State;role:Role;refresh:()=>Promise<void>;compact?:boolean}){
  ...
  async function decide(id:number,status:"accepted"|"rejected"){
    ...
    setError(false);
    try{
      const response=await apiFetch("/api/state",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"proposalStatus",id,status})});
  ...
  return <div>
    {!compact&&<><span className="eyebrow">{t.proposals}</span><h1 className="mt-4 text-4xl font-black">{role==="business"?t.incoming:t.myProposals}</h1><p className="mt-3 text-slate-600">{role==="business"?t.incomingText:t.studentProposalText}</p></>}
    {error&&<p role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-red-700">{t.error}</p>}
    <div className={compact?"space-y-4":"mt-8 space-y-4"}>
      {!state.proposals.length&&<div className="card p-10 text-center text-slate-500">{role==="business"?t.empty:t.studentEmpty}</div>}
      {state.proposals.map(proposal=>{
        const task=state.tasks.find(item=>item.id===proposal.taskId);
        const status=proposal.status as "accepted"|"rejected"|"pending";
        return <article key={proposal.id} className="card p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><span className="tag">{proposal.teamName}</span><h2 className="mt-3 text-xl font-black">{task?.title??`#${proposal.taskId}`}</h2></div>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${statusClass[status]}`}>{t[status]}</span>
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <div className="rounded-xl bg-slate-50 p-4 md:col-span-2"><p className="label">{t.idea}</p><p className="text-sm leading-6 text-slate-700">{proposal.solutionIdea}</p></div>
            <div className="rounded-xl bg-slate-50 p-4 md:col-span-2"><p className="label">{t.plan}</p><p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{proposal.plan}</p></div>
            <div className="flex items-center gap-2 text-sm text-slate-600"><CalendarDays size={17}/><b>{t.duration}:</b> {proposal.estimatedDuration}</div>
            <a className="flex items-center gap-2 break-all text-sm font-bold text-indigo-600 hover:underline" href={proposal.prototypeUrl} target="_blank" rel="noreferrer"><ExternalLink size={17}/>{t.viewPrototype}</a>
          </div>
          <p className="mt-5 text-xs text-slate-400">{t.submitted}: {formatter.format(new Date(proposal.createdAt))}</p>
          {role==="business"&&status==="pending"&&<div className="mt-5 flex flex-wrap gap-2">
            <button disabled={busyId!==null} onClick={()=>decide(proposal.id,"accepted")} className="primary">{busyId===proposal.id?<Loader2 className="animate-spin"/>:<Check/>}{t.accept}</button>
            <button disabled={busyId!==null} onClick={()=>decide(proposal.id,"rejected")} className="secondary">{t.reject}</button>
          </div>}
        </article>;
      })}
    </div>
