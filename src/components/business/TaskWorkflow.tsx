...
export default function TaskWorkflow({ locale, onCatalog }: { locale: Locale; onCatalog: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    api<{ tasks: TaskDraft[] }>("/api/tasks").then(data => { if (active) setDrafts(data.tasks); }).catch(() => { if (active) setError("load_failed"); });
    return () => { active = false; };
  }, []);
  ...
  return <section className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><span className="eyebrow">AI Task Builder</span><h1 className="mt-3 text-3xl font-black">{ru ? "Создайте понятную бизнес-задачу" : "Түсінікті бизнес міндетін жасаңыз"}</h1></div><button className="secondary" disabled={busy} onClick={() => load(null)}>{ru ? "Новая задача" : "Жаңа міндет"}</button></div>
